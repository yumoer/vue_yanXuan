  ...
  <div class="geren_Container">
    ...
    <div class="container_content">
      ...
      <div class="content_center">
        <div class="center_phone" @click="goLogin(false)">
          <i class="iconfont icon-shouji"></i>
          <span >手机号码登录</span>
        </div>
        <div class="center_email" @click="goLogin(true)">
          <i class="iconfont icon-email"></i>
          <span>邮箱帐号登录</span>
        </div>
        <div class="center_likelogin">
          <span>手机号快捷注册</span>
          <i class="iconfont icon-jiantou1"></i>
        </div>
      </div>
      <ul class="content_footer">
        <li class="chatType Wechat">
          <span class=" chat_item weixin_item">
            <i class="iconfont icon-weixin icon"></i>
            <span class=" chat wexin">微信</span>
          </span>
        </li>
        <li class="chatType QQ">
          <span class="chat_item  wechat_item">
            <i class="iconfont icon-weixin icon"></i>
            <span class="chat qq">QQ</span>
          </span>
        </li>
        <li class="chatType Weibo">
          <span class="chat_item wechat_item">
            <i class="iconfont icon-webo icon"></i>
            <span class="chat weibo">微博</span>
          </span>
        </li>
      </ul>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      goLogin(newLoginWay){
        //this.loginWay = newLoginWay
        console.log(this.loginWay)
        //Vue.set(this.loginWay,'loginWay')
        this.$router.push(`/login`)
        this.$store.dispatch('setLoginWay',newLoginWay)
        // vue.setState(this.loginWay)
      }
    },
